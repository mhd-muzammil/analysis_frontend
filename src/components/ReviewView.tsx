...
export default function ReviewView() {
  ...
  const handleMetricClick = (label: string) => {
    ...
    switch (label) {
      case 'Engineer Count': items = Array.from(new Set(activeRows.map(r => r.engg).filter(e => e && e.trim() !== ''))).sort(); break;
      case 'Total Flex Call': items = rows.map(r => r.ticketNo); break;
      case 'WIP Call': items = activeRows.filter(r => r.morningStatus.toLowerCase() !== 'closed').map(r => r.ticketNo); break;
      case 'Actionable Calls': items = activeRows.filter(r => r.morningStatus.toLowerCase() === 'actionable').map(r => r.ticketNo); break;
      case 'Trade WIP Call': items = activeRows.filter(r => r.segment.toLowerCase() === 'trade').map(r => r.ticketNo); break;
      case 'Closed(OTB)': items = droppedRows.map(r => r.ticketNo); break;
    }
    if (items.length > 0) setActiveDetail({ label, items });
  };

  const [newRow, setNewRow] = useState<Partial<ClassifiedRow>>({
    classification: 'NEW', month: '', ticketNo: '', woOtcCode: '', caseId: '',
    product: '', wipAging: 0, location: '', segment: 'Trade', hpOwner: 'Manual',
    flexStatus: 'Manual Entry', morningStatus: 'Actionable', eveningStatus: '',
    currentStatusTAT: '', engg: '', contactNo: '', parts: ''
  });
  ...
  const handleFlexUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (file.name.endsWith('.csv')) data = await parseCSV(file);
        ...
        let name = parsed.sheets.find(s => s.toLowerCase() === 'data') || parsed.sheets.reduce((b, s) => ((parsed.data[s] || []).length > (parsed.data[b] || []).length ? s : b), parsed.sheets[0]);
        data = parsed.data[name] || [];
      }
      setFlexData(data, detectCities(data)); setFlexFile(file);
  ...
  const handleYestUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0]; if (!file) return; setError(null);
    try {
      const parsed = await parseXLSX(file); const name = findOpenCallSheet(parsed.sheets);
      if (!name) throw new Error('Sheet missing.'); setYesterdayData(parsed.data[name]); setYestFile(file);
  ...
  const handleExportSummary = () => {
    try { exportSummaryXLSX(rows, engineers.filter(e => e.trim() !== '').length, selectedCity, reportDate); } catch { alert("Failed."); }
  };

  const handleExportCallPlan = async () => {
    try {
      exportCallPlanXLSX(rows, droppedRows, selectedCity, reportDate);
      const apiRows: ApiRow[] = rows.map(r => ({ ticket_no: r.ticketNo, case_id: r.caseId, product: r.product, wip_aging: r.wipAging, location: r.location, segment: r.segment, classification: r.classification, morning_status: r.morningStatus, evening_status: r.eveningStatus, engineer: r.engg, contact_no: r.contactNo, parts: r.parts, month: r.month, wo_otc_code: r.woOtcCode, hp_owner: r.hpOwner, flex_status: r.flexStatus, wip_changed: r.wipChanged, current_status_tat: r.currentStatusTAT }));
  ...
  const handleAddManualRow = () => {
    if (!newRow.ticketNo) { alert("Ticket No required"); return; }
    addRow(newRow as ClassifiedRow); setIsModalOpen(false);
    setNewRow({ classification: 'NEW', month: '', ticketNo: '', woOtcCode: '', caseId: '', product: '', wipAging: 0, location: '', segment: 'Trade', hpOwner: 'Manual', flexStatus: 'Manual Entry', morningStatus: 'Actionable', eveningStatus: '', currentStatusTAT: '', engg: '', contactNo: '', parts: '' });
  };

  const totalFlexCalls = flexData?.length || 0;
  const boxMetrics = [
    { label: 'Total Flex Call', value: totalFlexCalls, icon: UploadCloud, color: 'text-blue-400', bg: 'bg-blue-500/10' },
    { label: 'Engineer Count', value: engineers.filter(e => e.trim() !== '').length, icon: Sparkles, color: 'text-pink-400', bg: 'bg-pink-500/10' },
    { label: 'WIP Call', value: rows.length, icon: FileDown, color: 'text-blue-400', bg: 'bg-blue-500/10' },
    { label: 'Actionable Calls', value: rows.filter(r => r.morningStatus.toLowerCase() === 'actionable').length, icon: AlertCircle, color: 'text-green-400', bg: 'bg-green-500/10' },
    { label: 'Trade WIP Call', value: rows.filter(r => r.segment.toLowerCase() === 'trade').length, icon: Sparkles, color: 'text-amber-400', bg: 'bg-amber-500/10' },
  ...
  const tableMetrics = [
    ...
    { label: 'Closed Calls', value: rows.filter(r => r.morningStatus.toLowerCase() === 'closed').length },
    { label: 'Engg onsite', value: rows.filter(r => r.morningStatus.toLowerCase() === 'engg onsite').length },
    { label: 'To be schedule', value: rows.filter(r => r.morningStatus.toLowerCase() === 'to be scheduled').length },
    { label: 'CX Reschedule Calls', value: rows.filter(r => r.morningStatus.toLowerCase() === 'cx reschedule' || r.morningStatus.toLowerCase() === 'cx pending').length },
    { label: 'SSC Pending Calls', value: rows.filter(r => r.morningStatus.toLowerCase() === 'ssc pending').length },
    { label: 'Elevate/Tech Support Calls', value: rows.filter(r => r.morningStatus.toLowerCase() === 'elevate/tech support').length },
    { label: 'Under observation Calls', value: rows.filter(r => r.morningStatus.toLowerCase() === 'under observation').length },
    { label: 'To be Yank', value: rows.filter(r => r.morningStatus.toLowerCase() === 'to be yank').length },
    { label: 'Closed cancelled', value: rows.filter(r => r.morningStatus.toLowerCase() === 'closed cancelled').length },
    { label: 'Add.Part ordered', value: rows.filter(r => r.morningStatus.toLowerCase() === 'additional part').length },
    { label: 'To be Cancel', value: rows.filter(r => r.morningStatus.toLowerCase().includes('cancel')).length },
    { label: 'New calls', value: rows.filter(r => r.classification === 'NEW').length },
    ...
    <div className="w-full flex-col space-y-7 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className={`glass-panel p-6 rounded-2xl border border-gray-700/50 transition-all duration-500 overflow-hidden ${(!result && rows.length === 0) || showImport ? 'max-h-[1000px]' : 'max-h-[70px] opacity-70 hover:opacity-100'}`}>
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-blue-500/10 rounded-lg"><UploadCloud className="h-5 w-5 text-blue-400" /></div>
            <h2 className="text-lg font-bold text-gray-100 italic uppercase">Operational Data</h2>
          </div>
          {(result || rows.length > 0) && <button onClick={() => setShowImport(!showImport)} className="text-xs font-bold text-blue-400 hover:text-blue-300 uppercase tracking-widest px-4 py-1.5 bg-blue-500/5 border border-blue-500/20 rounded-lg">{showImport ? 'Close' : 'Update Data'}</button>}
        </div>
        {((!result && rows.length === 0) || showImport) && (
          <div className="space-y-6 animate-in zoom-in-95 duration-300">
            {error && <div className="bg-red-500/10 border border-red-500/20 text-red-400 px-4 py-2 rounded-lg text-xs italic">{error}</div>}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div onClick={() => flexRef.current?.click()} className={`flex flex-col items-center justify-center p-6 rounded-xl border-2 border-dashed cursor-pointer transition-all ${flexFile ? 'bg-green-500/5 border-green-500/30' : 'bg-gray-900/50 border-gray-800 hover:border-blue-500/50'}`}>
                <input type="file" className="hidden" ref={flexRef} onChange={handleFlexUpload} accept=".csv,.xlsx" />
                {flexFile ? <div className="flex items-center gap-3"><CheckCircle className="h-5 w-5 text-green-400" /><span className="text-sm font-medium text-gray-200 truncate max-w-[150px]">{flexFile.name}</span></div> : <span className="text-sm text-gray-500">Upload Today's Flex WIP</span>}
              </div>
              <div onClick={() => yestRef.current?.click()} className={`flex flex-col items-center justify-center p-6 rounded-xl border-2 border-dashed cursor-pointer transition-all ${yestFile ? 'bg-green-500/5 border-green-500/30' : 'bg-gray-900/50 border-gray-800 hover:border-blue-500/50'}`}>
                <input type="file" className="hidden" ref={yestRef} onChange={handleYestUpload} accept=".xlsx" />
                {yestFile ? <div className="flex items-center gap-3"><CheckCircle className="h-5 w-5 text-green-400" /><span className="text-sm font-medium text-gray-200 truncate max-w-[150px]">{yestFile.name}</span></div> : <span className="text-sm text-gray-500">Upload Yesterday's Plan</span>}
              </div>
            </div>
      ...
      {(result || rows.length > 0) && (
        <>
          <div className="flex items-center justify-end glass-panel p-4 rounded-xl border border-gray-700/50 gap-3">
             <button onClick={loadHistory} className="flex items-center gap-2 text-sm px-4 py-2 border border-gray-600/30 rounded-lg text-gray-400"><History className="h-4 w-4" /> History</button>
             <button onClick={handleExportSummary} className="flex items-center gap-2 text-sm px-4 py-2 border border-blue-500/30 rounded-lg text-blue-400"><Download className="h-4 w-4" /> EXPORT TN</button>
             <button onClick={handleExportCallPlan} className="flex items-center gap-2 text-sm px-5 py-2 bg-blue-600 hover:bg-blue-500 rounded-lg text-white shadow-lg shadow-blue-900/20"><Download className="h-4 w-4" /> Download Call Plan</button>
          </div>
          ...
          <div className="flex flex-col xl:flex-row gap-6 items-start">
            <div className="w-full xl:w-[48%] flex-shrink-0">
              <div className="overflow-hidden rounded-xl border border-gray-700/50 shadow-2xl">
                 ...
                 <div className="bg-[#FFFF00] grid grid-cols-[50px_1fr_80px] text-black font-black text-[10px] uppercase border-y border-black/10">
                    <div className="px-3 py-1 border-r border-black/10 text-center">S.No</div>
                    <div className="px-4 py-1 border-r border-black/10">Description</div>
                    <div className="px-3 py-1 text-center">Count</div>
                 </div>
                 <div className="bg-white/95 text-black">
                    {tableMetrics.map((m, idx) => (
                      <div key={idx} className={`grid grid-cols-[50px_1fr_80px] border-b border-gray-300 last:border-b-0 ${[5, 14, 16].includes(idx) ? 'bg-[#FFFF00]/10' : ''}`}>
                         <div className="px-3 py-0.5 border-r border-gray-300 text-center text-[10px]">{idx + 1}</div>
                         <div className="px-4 py-0.5 border-r border-gray-300 font-bold text-xs">{m.label}</div>
                         <div className={`px-3 py-0.5 text-center font-black text-xs ${[5, 14, 16].includes(idx) ? 'bg-[#FFFF00]' : ''}`}>{m.value > 0 ? m.value : ''}</div>
                      </div>
                    ))}
                 </div>
              </div>
            ...
            <div className="flex-1 w-full space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {boxMetrics.map((m, i) => (
                  <div key={i} onClick={() => handleMetricClick(m.label)} className="glass-panel p-3.5 rounded-2xl flex flex-col items-center justify-center text-center cursor-pointer border border-gray-700/30 hover:border-blue-500/50 transition-all hover:scale-[1.02]">
                    <div className={`p-2 rounded-xl mb-2 ${m.bg}`}><m.icon className={`h-4 w-4 ${m.color}`} /></div>
                    <p className="text-gray-400 text-[9px] font-bold uppercase tracking-widest">{m.label}</p>
                    <h4 className="text-xl font-black text-gray-100">{m.value}</h4>
                  </div>
                ))}
              </div>
              {sortedEnggs.length > 0 && (
                <div className="space-y-3 p-4 glass-panel rounded-2xl border border-gray-700/30">
                  <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-500 ml-1">Engg Allocation</h3>
                  <div className="flex flex-wrap gap-2">
                    {sortedEnggs.map(([eng, count]) => (
                      <div key={eng} className="px-3 py-1.5 bg-gray-800/60 rounded-lg border border-white/5 flex items-center gap-2 hover:border-blue-500/30 transition-all">
                        <span className="text-[11px] font-bold text-gray-300">{eng}</span>
                        <span className="text-[11px] font-black text-blue-400 bg-blue-500/10 px-1.5 rounded">{count}</span>
                      </div>
                    ))}
                  </div>
                </div>
               ...
               {(['all', 'actionable', 'planned', 'trade', 'pending', 'new', 'dropped'] as const).map(tab => (
                 <button key={tab} onClick={() => setActiveTab(tab)} className={`px-5 py-3 text-[11px] font-black uppercase tracking-widest relative ${activeTab === tab ? 'text-white' : 'text-gray-500 hover:text-gray-300'}`}>
                   {tab === 'all' ? 'All Open Call' : 
                    ...
                    tab === 'pending' ? 'Pending Call' : 
                    tab === 'new' ? 'New Call' : 'Closed Call (OTB)'}
                   {activeTab === tab && <div className="absolute bottom-0 left-0 w-full h-[2px] bg-blue-500 shadow-[0_0_8px_rgba(59,130,246,0.5)]" />}
                 </button>
               ))}
             </div>
             <div className="p-1">
               <DataTable 
                 data={
                   activeTab === 'all' ? rows : 
                   activeTab === 'actionable' ? rows.filter(r => r.morningStatus.toLowerCase() === 'actionable') :
                   activeTab === 'planned' ? rows.filter(r => r.engg && r.engg.trim() !== '') :
                   activeTab === 'trade' ? rows.filter(r => r.segment.toLowerCase() === 'trade') :
      ...
      {activeDetail && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
           <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={() => setActiveDetail(null)} />
           <div className="glass-panel w-full max-w-md relative z-10 rounded-3xl overflow-hidden border border-gray-700/50 shadow-2xl animate-in zoom-in-95 duration-300">
              <div className="bg-gray-900/80 px-6 py-4 flex items-center justify-between border-b border-gray-800">
                <div className="flex items-center gap-3">
                   <div className="p-2 bg-blue-500/10 rounded-lg"><Sparkles className="h-4 w-4 text-blue-400" /></div>
                   <div><h3 className="text-sm font-bold text-gray-100 italic uppercase">{activeDetail.label}</h3><p className="text-[10px] text-gray-500 underline decoration-blue-500/20">{activeDetail.items.length} items found</p></div>
                </div>
                <button onClick={() => setActiveDetail(null)} className="p-1.5 hover:bg-white/10 rounded-full"><X className="h-4 w-4 text-gray-400" /></button>
              </div>
              <div className="max-h-[400px] overflow-y-auto p-4 custom-scrollbar bg-gray-900/40">
                {activeDetail.items.map((item, idx) => (
                  <div key={idx} className="p-3 bg-white/5 border border-white/5 rounded-xl mb-2 flex items-center gap-3">
